...
없을 때 MIT 라이선스의 고정된 `facebook/m2m100_418M` 한→영 도우미 약
1.9 GB를 함께 준비합니다. 도우미 revision은
`55c2e61bbf05dfb8d7abccdc3fae6fc8512fd636`으로 고정됩니다.
이미 캐시된 이미지 모델도 새 Studio 프로세스의 준비 단계에서 검토된
크기와 SHA-256을 스트리밍 검증한 뒤에만 로드합니다. T4에서 FLUX 약
16.2 GB 캐시를 확인할 때는 다운로드가 없어도 최대 약 1분 걸릴 수 있고,
같은 프로세스에서는 검증 영수증을 즉시 재사용합니다.
...
번역 전에 결정론적 용어집으로 인식 가능한 시각·제품 디자인 용어를
보호합니다. 이 과정은 재료·색상 같은 용어가 엉뚱하게 직역되는 경우를
줄이기 위한 제한된 보조 수단이며, 모든 한국어 문장의 완벽한 번역을
보장하지 않습니다. 한국어 원문, 준비된 영어, 도우미 ID와 고정 revision은
각각 provenance로 남습니다.
...
16.2 GB이고 한→영 도우미가 없으면 약 1.9 GB가 추가되며, 이후에는 같은
...
Studio를 막 열었을 때 기존 이미지 파일이 **아직 미검증 · 없으면
다운로드**로 보일 수 있습니다. 이는 보수적인 최대 전송량 표시이며
재다운로드 확정이 아닙니다. **안내 확인 후 모델 준비**가 크기와 SHA-256을
검증하면 같은 프로세스에서 0 B 캐시로 갱신됩니다.
...
적용됩니다. 한→영 도우미는 고정 revision의 Transformers snapshot을
사용합니다. 허용된 런타임 파일 7개 각각의 크기와 SHA-256을 검증하고,
파일 경계와 스트리밍 해시 청크 사이에서 중지를 확인합니다.
